...
<head>
    ...
    <style>
        ...
        .booking-dialog{position:fixed;inset:0;margin:auto;max-width:calc(100vw - 24px)}.dialog-inner,.dialog-edit-form,.layout>*,.card,.two>div{min-width:0}.dialog-inner{width:100%;overflow:hidden}input,select,textarea{display:block;inline-size:100%;max-inline-size:100%;min-inline-size:0}@media(max-width:800px){.layout{grid-template-columns:minmax(0,1fr)}.card{width:100%;max-width:100%;overflow:hidden}.two{grid-template-columns:minmax(0,1fr)}input[type=date],input[type=time]{width:100%;min-width:0;max-width:100%;-webkit-appearance:none;appearance:none}.booking-dialog{inset:0;width:calc(100vw - 24px);max-width:calc(100vw - 24px);max-height:calc(100dvh - 24px);margin:auto}}
    </style>
</head>
<body>
@php
                    ...
                    <article class="booking {{ $booking->cancelled_at?'cancelled':($booking->confirmed_at?'':'pending') }}">
                        ...
                        <div class="booking-actions">
                            @if(!$booking->confirmed_at && !$booking->cancelled_at)
                                <button class="action-button action-confirm modal-trigger" type="button" data-dialog-open="edit-{{ $booking->id }}">Conferma prenotazione</button>
                            @endif
                        ...
                        <dialog id="edit-{{ $booking->id }}" class="booking-dialog">
                            <div class="dialog-inner">
                                <div class="dialog-top"><h3>{{ !$booking->confirmed_at && !$booking->cancelled_at ? 'Completa e conferma prenotazione' : 'Modifica prenotazione' }}</h3><button class="dialog-close" type="button" data-dialog-close aria-label="Chiudi">×</button></div>
                                <form class="dialog-edit-form" method="post" action="{{ route('agenda.update',$booking) }}">
                                    ...
                                    <div class="dialog-buttons"><button class="dialog-back" type="button" data-dialog-close>Indietro</button><button class="{{ !$booking->confirmed_at && !$booking->cancelled_at ? 'dialog-confirm' : 'dialog-edit' }}" type="submit">{{ !$booking->confirmed_at && !$booking->cancelled_at ? 'Conferma prenotazione' : 'Salva modifica' }}</button></div>
                                </form>
                            </div>
                        </dialog>
                        @if(!$booking->cancelled_at)
