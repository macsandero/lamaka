                ...
                <div class="calendar">
                    @for($day=$start; $day->lte($end); $day=$day->addDay())
                        ...
                        <a class="day {{ !$day->isSameMonth($month)?'muted':'' }} {{ $date===$selectedDate?'selected':'' }} {{ $stats['closing']<0?'negative':'' }}" href="{{ route('uova.index',['date'=>$date,'month'=>$day->format('Y-m')]) }}#giorno">
                            ...
                            @foreach($items->take(3) as $item)<span class="dot {{ $item->is_collected?'done':'' }}">{{ $item->creator_initial ? $item->creator_initial.' · ' : '' }}{{ $item->contact->full_name }} · {{ $item->quantity }}</span>@endforeach
                        </a>
                    @endfor
                </div>
            </div>
